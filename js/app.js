...
function renderDropdownResults() {
    ...
    // Filter matches: if query is empty, show first 50 items for easy browsing
    let matches = [];
    if (!query) {
    ...
    if (matches.length > 0) {
        ...
        matches.forEach(match => {
            ...
            // Use mousedown to execute selection before input blur hides the dropdown
            item.addEventListener("mousedown", (e) => {
                e.preventDefault(); // Keep input focused
                searchInput.value = currentLang === "ta" ? match.ta : match.en;
                resultsDiv.style.display = "none";
                
                // Trigger input event to show action buttons
                searchInput.dispatchEvent(new Event("input"));
                
                // Add the product directly on selection
                addProductAction();
            });
...
function initProductSelector() {
    ...
    if (!searchInput || !resultsDiv || !whatsappBtn || !addProductBtn || !clearProductBtn || !window.productsData) return;

    // Toggle Plus (+) and Close (x) buttons on user input
    ...
    searchInput.addEventListener("focus", renderDropdownResults);
    searchInput.addEventListener("input", renderDropdownResults);

    // Hide dropdown suggestions on focus out
